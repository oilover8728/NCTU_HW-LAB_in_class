...
class ADXL345(IMU):
    ...
    def getPitch(self) :
        ...
        aY = self.getYg()
        aZ = self.getZg()
        self.pitch = atan(-aX/sqrt(aY*aY+aZ*aZ))*180/pi
        return self.pitch
...
try:
    ...
    pitch1=adxl345.getPitch()
    roll1=adxl345.getRoll()
    time.sleep(1)
    while(True):
        gyro.getXangle()
        gyro.getYangle()
        ...
        pitch2 = (pitch1+gyro.getXangle())*0.98 + adxl345.getPitch() * 0.02
        print ("pitch = %.3f" % (pitch2))
        print ("")
        pitch1 = adxl345.getPitch()
        
        roll2 = (roll1+gyro.getYangle())*0.98 + adxl345.getRoll() * 0.02
        print ("roll = %.3f" % (roll2))
        print ("")
        roll1 = adxl345.getRoll()
        time.sleep(1)
